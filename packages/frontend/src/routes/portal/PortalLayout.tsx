...
export function PortalLayout() {
  ...
  const { user, lastFirmId, logout, bootstrap, isBootstrapped } =
    usePortalAuthStore();
  const navigate = useNavigate();
  const matches = useMatches();
  const footerLinks = buildPortalShellFooterLinks(t);
  const inCasePage = matches.some((match) =>
    /^\/portal\/cases\/[^/]+$/.test(match.pathname)
  );
  ...
  useEffect(() => {
    ...
    if (lastFirmId) {
      void navigate({
        to: "/portal/$firmId/login",
        params: { firmId: lastFirmId },
        replace: true
      });
      return;
    }
              ...
              {breadcrumbItems.map((item, index) => (
                ...
                >
                  ...
                  {item.to ? (
                    <Link
                      to={item.to}
                      className="transition hover:text-accent hover:underline underline-offset-2"
                    >
                      {item.label}
                    </Link>
                  ) : (
